...
The tmux status bar reports CPU use, memory use, and download/upload rates once
per second. Memory use excludes memory the kernel reports as available,
including reclaimable cache. Network traffic follows the default-route
interface. To select an interface explicitly, add this after the source line
in `~/.tmux.conf`:
